...
def next_state(state):
    ...
    for i in range(9):
        if nextState.state[i] == '-':
            nextState.state[i] = state.currentPlayer
            newMoveVal = minimax(nextState)
            nextState.state[i] = '-'
            if state.currentPlayer == 'X' and newMoveVal > score_val:
                score_val = newMoveVal
                bestStateIndex = i

            elif state.currentPlayer == 'O' and newMoveVal < score_val:
                score_val = newMoveVal
                bestStateIndex = i
    if bestStateIndex != -1:
        nextState.state[bestStateIndex] = state.currentPlayer
